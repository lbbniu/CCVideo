...
- (void)layoutSubviews
{
    ...
    if (self.playState == ZXTimeIndicatorPlayStateRewind) {
        ...
        [self.arrowImageView setImage:[UIImage imageNamed:@"res_lbbVideo/zx-video-player-rewind"]];
        
    } else {
        
        [self.arrowImageView setImage:[UIImage imageNamed:@"res_lbbVideo/zx-video-player-fastForward"]];
    }
}
...
- (void)createTimeIndicator
{
    ...
    _arrowImageView = [[UIImageView alloc] initWithFrame:CGRectMake((kVideoTimeIndicatorViewSide - 50) / 2, margin, 50, 30)];
    [self addSubview:_arrowImageView];
    
    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, margin + 35 + kViewSpacing, kVideoTimeIndicatorViewSide, 12)];
